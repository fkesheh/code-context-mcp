...
import fs from "fs";
import dbInterface, { initializeDatabase } from "./db.js";
import {
  cloneRepository,
  getRepositoryFiles,
  processFileContents,
...
import { getDefaultBranch } from "./githubRepo.js";

// Define input schemas for tools
const QueryRepoSchema = z.object({
  repoUrl: z.string().describe("URL of the git repository"),
  branch: z.string()
    .optional()
  ...
  keywords: z.array(z.string())
    .optional()
    .describe("Optional array of keywords to filter results (results must contain at least one keyword)"),
  limit: z
    .number()
    .optional()
    .default(5)
...
export const createServer = () => {
  ...
  // Initialize database during server startup
  const initDb = async () => {
    if (!dbInitialized) {
      db = dbInterface;
      initializeDatabase();
  ...
  // Set up logging level handler
  server.setRequestHandler(SetLevelRequestSchema, async (request) => {
    logLevel = request.params.level;
    return { success: true };
  });

  // Set up tool schema for the server
      ...
      tools: [
        {
          ...
          description:
            "Query a GitHub repository for code matching a search query",
  ...
  // Implement ListToolsRequestSchema handler for tools/list method
    ...
    switch (name) {
      case ToolName.QUERY_REPO: {
        try {
          ...
          const { repoUrl, branch, query, limit, keywords } = input as z.infer<
            typeof QueryRepoSchema
          >;
          ...
          if (!repoUrl || !query) {
            ...
            return {
              error: {
                message:
                  "Required parameters (repoUrl, query) are missing",
              },
            };
          }
          ...
          if (!branch) {
            ...
            try {
              ...
              console.error(`[queryRepo] Getting default branch for ${owner}/${repo}`);
          ...
          if (repoExists) {
            ...
            // Register repository
            console.error(
              `[queryRepo] Registering new repository: ${repoName}`
            );
            ...
            console.error(
              `[queryRepo] Repository registered with ID: ${repoId}`
            );
          }
          ...
          if (branchExists) {
            ...
            if (branchExists.last_commit_sha !== latestCommit) {
              ...
              console.error(
                `[queryRepo] Commit SHA unchanged, skipping update`
              );
            }
          } else {
            // Register the branch
          ...
          if (needsUpdate) {
            console.error(
              `[queryRepo] Processing repository files (${
                Date.now() - startTime
              }ms)`
            );
            ...
            console.error(
              `[queryRepo] Found ${files.length} files in repository`
            );
            ...
            const processFiles = (db: any) => {
              ...
              for (const [path, file] of existingFileMap.entries()) {
                ...
                if (!fileStillInUse) {
                  // Delete chunks first
                  db.prepare("DELETE FROM file_chunk WHERE file_id = ?").run(
                    file.id
                  );
            ...
            const filesToProcess = dbInterface.transaction((db) =>
              processFiles(db)
            );
            console.error(
              ...
              try {
                ...
                if (fs.existsSync(filePath)) {
                  ...
                  const extension = path
                    ...
                    .toLowerCase()
                    .slice(1);
                  const splitterType = extensionToSplitter(extension);
                  console.error(
                  ...
                  for (
                    ...
                    const embeddings = await generateHuggingFaceEmbeddings(
                      chunkContents
                    );
                    console.error(
                    ...
                    dbInterface.transaction((db) => {
                      const insertChunkStmt =
                        db.prepare(`INSERT INTO file_chunk (
                        file_id, content, chunk_number, embedding, model_version, token_count
                      ) VALUES (?, ?, ?, ?, 'huggingface-Salesforce-SFR-Embedding-Code-400M_R', ?)`);
          ...
          console.error(
            `[queryRepo] Generating embedding for query: "${query}"`
          );
          ...
          const results = dbInterface.all(
            ...
            LIMIT ?
          `,
            [queryEmbeddingStr, branchId, limit]
          );
          console.error(
            `[queryRepo] Search completed in ${
              Date.now() - searchStart
            }ms, found ${results.length} results`
          );

          // Filter results by keywords if provided
          let filteredResults = results;
          if (keywords && keywords.length > 0) {
            console.error(`[queryRepo] Filtering results by keywords: ${keywords.join(', ')}`);
            ...
            console.error(
              ...
              }ms, filtered from ${results.length} to ${filteredResults.length} results`
            );
          }
